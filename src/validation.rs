...
impl ValidationReport {
    ...
    pub fn print_summary(&self) {
        ...
        eprintln!(
            ...
            self.issues.len(), errors, warnings, infos
        );
        ...
        if self.issues.is_empty() {
            ...
            for issue in &self.issues {
                eprintln!("  [{}] {:35}  {}", issue.severity.label(), issue.check, issue.message);
            }
        }
        eprintln!();
    }
...
fn check_system(r: &mut ValidationReport, n: &Network) {
    ...
    if n.case_id.sbase <= 0.0 {
        push(r, Severity::Error, "MMWG-7.3.1/sbase-nonpositive",
            format!("SBASE = {} is non-positive; a positive MVA base is required", n.case_id.sbase));
    } else if n.case_id.sbase < 1.0 || n.case_id.sbase > 100_000.0 {
        push(r, Severity::Warning, "MMWG-7.3.1/sbase-unusual",
            format!("SBASE = {} MVA is outside the typical range [1, 100000]; verify case identification record",
                n.case_id.sbase));
    }

    // BASFRQ must be 50 or 60 Hz
    if n.case_id.basfrq != 50.0 && n.case_id.basfrq != 60.0 {
        push(r, Severity::Warning, "MMWG-7.3.1/basfrq-unusual",
            format!("BASFRQ = {} Hz is not 50 or 60 Hz; verify frequency base", n.case_id.basfrq));
    }

    // At least one slack bus required
    let slack_count = n.buses.iter().filter(|b| b.ide == BusType::Slack).count();
    if slack_count == 0 {
        push(r, Severity::Error, "MMWG-7.3.1/no-slack",
            "No slack (swing) bus found; at least one bus with IDE=4 is required".to_string());
    } else if slack_count > 1 {
        push(r, Severity::Info, "MMWG-7.3.1/multi-slack",
            format!("{slack_count} slack buses found; verify that multiple swing buses are intentional"));
    }

    // Case must have at least some buses
    if n.buses.is_empty() {
        push(r, Severity::Error, "MMWG-7.3.1/no-buses",
...
fn check_buses(r: &mut ValidationReport, n: &Network) {
    ...
    for bus in &n.buses {
        ...
        if !seen_ids.insert(bus.i) {
            push(r, Severity::Error, "MMWG-7.3.2/duplicate-bus-id",
                format!("Duplicate bus number {}", bus.i));
        }

        // BASKV must be positive
        if bus.baskv <= 0.0 {
            push(r, Severity::Error, "MMWG-7.3.2/baskv-nonpositive",
                format!("Bus {} '{}': BASKV = {} kV is non-positive", bus.i, bus.name, bus.baskv));
        }

        // Voltage magnitude out of realistic range [0.5, 1.5] pu
        if bus.vm < 0.5 || bus.vm > 1.5 {
            push(r, Severity::Warning, "MMWG-7.3.2/vm-out-of-range",
                format!("Bus {} '{}': VM = {:.4} pu is outside [0.50, 1.50]; solved case may have diverged",
                    bus.i, bus.name, bus.vm));
        }

        // Voltage limits sanity
        if bus.nvhi > 0.0 && bus.nvlo > 0.0 && bus.nvhi <= bus.nvlo {
            push(r, Severity::Warning, "MMWG-7.3.2/vhi-le-vlo",
                format!("Bus {} '{}': NVHI ({:.3}) ≤ NVLO ({:.3}); voltage limits are inverted",
                    bus.i, bus.name, bus.nvhi, bus.nvlo));
        }
        if bus.evhi > 0.0 && bus.evlo > 0.0 && bus.evhi <= bus.evlo {
            push(r, Severity::Warning, "MMWG-7.3.2/evhi-le-evlo",
                format!("Bus {} '{}': EVHI ({:.3}) ≤ EVLO ({:.3}); emergency voltage limits are inverted",
...
fn check_branches(r: &mut ValidationReport, n: &Network) {
    ...
    for br in &n.branches {
        ...
        if !bus_ids.contains(&br.i) {
            push(r, Severity::Error, "MMWG-7.3.3/orphan-branch-from",
                format!("Branch {}-{} ckt '{}': from-bus {} not in bus table",
                    br.i, br.j, br.ckt, br.i));
        }
        if !bus_ids.contains(&br.j) {
            push(r, Severity::Error, "MMWG-7.3.3/orphan-branch-to",
                format!("Branch {}-{} ckt '{}': to-bus {} not in bus table",
        ...
        if br.x.abs() > 1e-9 && (br.r / br.x).abs() > 5.0 {
            push(r, Severity::Warning, "MMWG-7.3.3/high-rx-ratio",
                format!("Branch {}-{} ckt '{}': R/X = {:.2}; unusually high for transmission",
                    br.i, br.j, br.ckt, br.r / br.x));
        }

        // Negative ratings
        if br.ratea < 0.0 || br.rateb < 0.0 || br.ratec < 0.0 {
            push(r, Severity::Error, "MMWG-7.3.3/negative-rating",
                format!("Branch {}-{} ckt '{}': negative thermal rating (RATEA={}, RATEB={}, RATEC={})",
    ...
    if zero_x_count > 0 {
        ...
        push(r, Severity::Warning, "MMWG-7.3.3/zero-x",
            format!("Branch(es) with X=0: {}{} — may cause numerical issues in full Newton-Raphson",
                examples, tail));
    }
}
...
fn check_transformers(r: &mut ValidationReport, n: &Network) {
    ...
    for tx in &n.transformers {
        ...
        if !bus_ids.contains(&tx.i) {
            push(r, Severity::Error, "MMWG-7.3.4/orphan-tx-from",
                format!("Transformer {}-{} ckt '{}': from-bus {} not in bus table",
                    tx.i, tx.j, tx.ckt, tx.i));
        }
        if !bus_ids.contains(&tx.j) {
            push(r, Severity::Error, "MMWG-7.3.4/orphan-tx-to",
                format!("Transformer {}-{} ckt '{}': to-bus {} not in bus table",
                    tx.i, tx.j, tx.ckt, tx.j));
        }

        // Zero leakage reactance
        if tx.x12 == 0.0 {
            push(r, Severity::Warning, "MMWG-7.3.4/zero-x12",
                format!("Transformer {}-{} ckt '{}': X1-2 = 0; potential numerical singularity",
                    tx.i, tx.j, tx.ckt));
        }

        // Non-positive SBASE1-2
        if tx.sbase12 <= 0.0 {
            push(r, Severity::Error, "MMWG-7.3.4/sbase12-nonpositive",
                format!("Transformer {}-{} ckt '{}': SBASE1-2 = {} is non-positive",
                    tx.i, tx.j, tx.ckt, tx.sbase12));
        }

        // Off-nominal turns ratio out of reasonable range
        if tx.windv1 < 0.5 || tx.windv1 > 2.0 {
            push(r, Severity::Warning, "MMWG-7.3.4/windv1-range",
                format!("Transformer {}-{} ckt '{}': WINDV1 = {:.4} is outside [0.5, 2.0]",
                    tx.i, tx.j, tx.ckt, tx.windv1));
        }
        if tx.windv2 < 0.5 || tx.windv2 > 2.0 {
            push(r, Severity::Warning, "MMWG-7.3.4/windv2-range",
                format!("Transformer {}-{} ckt '{}': WINDV2 = {:.4} is outside [0.5, 2.0]",
                    tx.i, tx.j, tx.ckt, tx.windv2));
        }

        // Phase shift > ±30° is highly unusual
        if tx.ang1.abs() > 30.0 {
            push(r, Severity::Warning, "MMWG-7.3.4/phase-shift-large",
                format!("Transformer {}-{} ckt '{}': ANG1 = {:.2}° exceeds ±30°; verify PST settings",
...
fn check_generators(r: &mut ValidationReport, n: &Network) {
    ...
    for machine in &n.generators {
        ...
        if !bus_ids.contains(&machine.i) {
            push(r, Severity::Error, "MMWG-7.3.5/orphan-gen",
                format!("Generator bus {} id '{}': bus not in bus table", machine.i, machine.id));
        }

        // Duplicate I/ID
        if !seen.insert((machine.i, machine.id.as_ref())) {
            push(r, Severity::Error, "MMWG-7.3.5/duplicate-gen-id",
                format!("Duplicate generator: bus {} id '{}'", machine.i, machine.id));
        }

        // MBASE must be positive
        if machine.mbase <= 0.0 {
            push(r, Severity::Error, "MMWG-7.3.5/mbase-nonpositive",
                format!("Generator bus {} id '{}': MBASE = {} is non-positive", machine.i, machine.id, machine.mbase));
        }

        // PT must be ≥ PB
        if machine.pt < machine.pb {
            push(r, Severity::Error, "MMWG-7.3.5/pt-lt-pb",
                format!("Generator bus {} id '{}': PT ({}) < PB ({}); active power limits are inverted",
                    machine.i, machine.id, machine.pt, machine.pb));
        }

        // QT must be ≥ QB
        if machine.qt < machine.qb {
            push(r, Severity::Error, "MMWG-7.3.5/qt-lt-qb",
                format!("Generator bus {} id '{}': QT ({}) < QB ({}); reactive power limits are inverted",
                    machine.i, machine.id, machine.qt, machine.qb));
        }

        // Voltage setpoint out of range [0.5, 1.5]
        if machine.vs < 0.5 || machine.vs > 1.5 {
            push(r, Severity::Warning, "MMWG-7.3.5/vs-out-of-range",
                format!("Generator bus {} id '{}': VS = {:.4} pu is outside [0.50, 1.50]",
        ...
        if machine.stat == 1 && machine.pt > machine.pb {
            if machine.pg > machine.pt + 1.0 {
                push(r, Severity::Warning, "MMWG-7.3.5/pg-above-pt",
                    format!("Generator bus {} id '{}': PG ({:.1}) > PT ({:.1}); dispatch exceeds max MW",
                        machine.i, machine.id, machine.pg, machine.pt));
            }
            if machine.pg < machine.pb - 1.0 {
                push(r, Severity::Warning, "MMWG-7.3.5/pg-below-pb",
                    format!("Generator bus {} id '{}': PG ({:.1}) < PB ({:.1}); dispatch below min MW",
...
fn check_loads(r: &mut ValidationReport, n: &Network) {
    ...
    for load in &n.loads {
        if !bus_ids.contains(&load.i) {
            push(r, Severity::Error, "MMWG-7.3.6/orphan-load",
        ...
        if load.status == 1 && total_p < -1.0 {
            push(r, Severity::Warning, "MMWG-7.3.6/negative-load-mw",
                format!("Load bus {} id '{}': total P = {:.1} MW is negative; intentional if modeling a generator as load",
                    load.i, load.id, total_p));
        }
    }

    if in_service_loads == 0 && !n.buses.is_empty() {
        push(r, Severity::Warning, "MMWG-7.3.6/no-in-service-loads",
...
fn check_fixed_shunts(r: &mut ValidationReport, n: &Network) {
    ...
    for sh in &n.fixed_shunts {
        if !bus_ids.contains(&sh.i) {
            push(r, Severity::Error, "MMWG-7.3.7/orphan-fixed-shunt",
                format!("Fixed shunt bus {} id '{}': bus not in bus table", sh.i, sh.id));
        }

        // Very large shunt values (> 10 000 MVAr) are likely data entry errors
        if sh.bl.abs() > 10_000.0 {
            push(r, Severity::Warning, "MMWG-7.3.7/large-bl",
                format!("Fixed shunt bus {} id '{}': |BL| = {:.1} MVAr is extremely large; verify scaling",
                    sh.i, sh.id, sh.bl));
        }
        if sh.gl.abs() > 10_000.0 {
            push(r, Severity::Warning, "MMWG-7.3.7/large-gl",
                format!("Fixed shunt bus {} id '{}': |GL| = {:.1} MW is extremely large; verify scaling",
                    sh.i, sh.id, sh.gl));
        }
    }
}
...
fn check_switched_shunts(r: &mut ValidationReport, n: &Network) {
    ...
    for ss in &n.switched_shunts {
        if !bus_ids.contains(&ss.i) {
            push(r, Severity::Error, "MMWG-7.3.8/orphan-switched-shunt",
        ...
        if ss.stat == 1 && ss.vswhi > 0.0 && ss.vswlo > 0.0 && ss.vswhi <= ss.vswlo {
            push(r, Severity::Warning, "MMWG-7.3.8/vswhi-le-vswlo",
                format!("Switched shunt bus {}: VSWHI ({:.3}) ≤ VSWLO ({:.3}); control window is inverted",
                    ss.i, ss.vswhi, ss.vswlo));
        }

        // Empty step list for a non-locked shunt
        if ss.modsw != 0 && ss.stat == 1 && ss.steps.is_empty() {
            push(r, Severity::Warning, "MMWG-7.3.8/no-steps",
                format!("Switched shunt bus {}: MODSW={} but step list is empty; shunt cannot switch",
                    ss.i, ss.modsw));
        }
    }
}
...
fn check_areas(r: &mut ValidationReport, n: &Network) {
    ...
    for area in &n.areas {
        if !seen_areas.insert(area.i) {
            push(r, Severity::Error, "MMWG-7.3.9/duplicate-area",
        ...
        if area.isw != 0 && !bus_ids.contains(&area.isw) {
            push(r, Severity::Error, "MMWG-7.3.9/area-swing-missing",
                format!("Area {} '{}': swing bus {} not in bus table", area.i, area.arnam, area.isw));
        }
    }
    ...
    if !orphan_areas.is_empty() {
        ...
        push(r, Severity::Warning, "MMWG-7.3.9/undefined-area",
...
fn check_system_balance(r: &mut ValidationReport, n: &Network) {
    let total_gen_mw: f64 = n.generators
        .iter()
        .filter(|g| g.stat == 1)
        .map(|g| g.pg)
        .sum();

    let total_load_mw: f64 = n.loads
        .iter()
        .filter(|l| l.status == 1)
        .map(|l| l.pl + l.ip + l.yp)
        .sum();

    let total_shunt_mw: f64 = n.fixed_shunts
        .iter()
        .filter(|s| s.status == 1)
        .map(|s| s.gl)
        .sum::<f64>()
    ...
    if total_gen_mw > 0.0 && total_load_mw > 0.0 && net_imbalance > threshold_mw {
        push(r, Severity::Warning, "MMWG-7.3.10/gen-load-imbalance",
            format!(
    ...
    if total_gen_mw <= 0.0 && !n.generators.is_empty() {
        push(r, Severity::Warning, "MMWG-7.3.10/zero-total-gen",
            format!("Total in-service generation = {:.1} MW; all generators may be offline or PG=0",
                total_gen_mw));
    }

    // System summary info
    push(r, Severity::Info, "MMWG-7.3.10/summary",
        format!(
            "buses={} branches={} transformers={} generators={} loads={} \
             gen_mw={:.0} load_mw={:.0} areas={} zones={} owners={}",
            n.buses.len(), n.branches.len(), n.transformers.len(),
            n.generators.len(), n.loads.len(),
            total_gen_mw, total_load_mw,
            n.areas.len(), n.zones.len(), n.owners.len()
        ));
}
...
fn push(r: &mut ValidationReport, severity: Severity, check: &'static str, message: String) {
    r.issues.push(ValidationIssue { severity, check, message });
}
